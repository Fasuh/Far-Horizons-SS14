...
public abstract class SharedLightGridSystem : EntitySystem
{
    ...
    protected virtual TimeSpan UpdateInterval => TimeSpan.FromSeconds(0.5f);
    ...
    protected const float MaxExposure = 3f;
    ...
    protected abstract void EnumeratePointLights();

    protected void ProcessLight(EntityUid lightUid, SharedPointLightComponent lightComp, TransformComponent xform)
    {
        if (HasComp<DarkLightComponent>(lightUid))
            return;
    ...
    protected static float DecodeLight(byte value)
    {
